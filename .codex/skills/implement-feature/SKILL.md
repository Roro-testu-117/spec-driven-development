...
- 本スキル内の `Read/Write/Edit/Grep/Glob/Bash/AskUserQuestion/Task` は**概念的なツール名**。Codexでは以下のように読み替える。
  - Read/Grep/Glob/Bash: `functions.shell_command`（`rg`/`ls`/`sed` 等）
  - Write/Edit: `functions.apply_patch`（単一ファイル編集）または `functions.shell_command`
  - AskUserQuestion: `functions.request_user_input`
- Codex CLIでは `Task` によるサブエージェント実行ができないため、**スキル内統合フローのみ**で運用する。
...
3. **context.md の生成**
   ...
   ```
   Read('.codex/skills/implement-feature/templates/context.md')
   ```
...
2. **requirements.md の生成**
   ...
   ```
   Read('.codex/skills/implement-feature/templates/requirements.md')
   ```
...
2. **design.md の生成**
   ...
   ```
   Read('.codex/skills/implement-feature/templates/design.md')
   ```
...
1. **tasklist.md の生成**
   ...
   ```
   Read('.codex/skills/implement-feature/templates/tasklist.md')
   ```

   **重要**: 冒頭の「運用ルール（Skills運用）」セクションは削除・改変しない

---

### スキル運用（統合フロー）

**このフローはスキル内で統合ロジックを直接実行します。**

#### 4-1. タスク分解（スキル内統合）

```
Read('.codex/skills/implement-feature/supporting-files/integrated-decomposer.md')
```
...
4. CTテンプレート: `.codex/skills/implement-feature/templates/ct-template.md`
...
```
Read('.codex/skills/implement-feature/supporting-files/integrated-executor.md')
```
...
**4-2-3. 品質レビュー**
```
Read('.codex/skills/implement-feature/supporting-files/integrated-reviewer.md')
```
...
- **OK の場合**:
  ...
  5. **ユーザーのコミット実行を確認してから**次の未完了CTへ（4-2-1に戻る）
...
**このステップ（スキル運用）が完了したら、ただちにステップ5に進む。**
